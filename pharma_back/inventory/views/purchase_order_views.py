...
def purchase_order_release_stock(request, order_id):
    """Releases stock after delivery - reduces supplier inventory and creates buyer inventory."""
    try:
        user = request.user
        organization_id = getattr(user, 'organization_id', None)
        branch_id = getattr(user, 'branch_id', None)
...
def purchase_order_import_stock(request, order_id):
    """Imports delivered stock to inventory with custom selling prices."""
    try:
        user = request.user
        organization_id = getattr(user, 'organization_id', None)
        branch_id = getattr(user, 'branch_id', None)
...
def purchase_order_ship(request, order_id):
    ...
    try:
        ...
        print(f"DEBUG SHIP: User {user.email}, org_id={organization_id}, branch_id={branch_id}, order_id={order_id}")
        ...
        try:
            ...
            if bulk_order:
                print(f"DEBUG SHIP: Order status: {bulk_order.status}")
                if bulk_order.status not in [BulkOrder.READY_TO_SHIP, BulkOrder.PAYMENT_COMPLETED, BulkOrder.BUYER_CONFIRMED]:
                    print(f"DEBUG SHIP: Status not ready for shipping")
                    return Response({'error': f'Order status {bulk_order.status} not ready for shipping'}, status=400)
            else:
                raise BulkOrder.DoesNotExist
                
        except BulkOrder.DoesNotExist:
            print(f"DEBUG SHIP: Order not found")
            return Response({'error': 'Order not found'}, status=404)
        ...
        print(f"DEBUG SHIP: Order updated successfully")
